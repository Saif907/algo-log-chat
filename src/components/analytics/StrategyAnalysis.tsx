...
export const StrategyAnalysis = ({ strategies, monthlyPerf }: Props) => {
  ...
  // Get unique strategy names for the legend/bars
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Card className="p-4">
          ...
          <p className={`text-2xl font-bold ${totalPnL >= 0 ? 'text-success' : 'text-destructive'}`}>${totalPnL.toLocaleString()}</p>
        </Card>
      </div>
          ...
          {strategies.map((strategy, index) => (
            <div key={strategy.name} className="p-3 sm:p-4 bg-muted/30 rounded-lg">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 text-[10px] sm:text-xs">
                ...
                <div>Max DD: <span className="text-destructive">${Math.abs(strategy.maxDrawdown).toFixed(0)}</span></div>
